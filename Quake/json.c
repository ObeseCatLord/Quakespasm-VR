...
/* Keep the Ironwail parser self-contained in this older common-code base. */
static size_t JSON_WriteUTF8 (char *dst, size_t maxbytes, unsigned int codepoint)
{
	if (!maxbytes)
		return 0;
	if (codepoint < 0x80)
	...
	if (codepoint < 0x800 && maxbytes >= 2)
	{
		dst[0] = 0xC0 | (codepoint >> 6);
		dst[1] = 0x80 | (codepoint & 63);
		return 2;
	}
	if (codepoint < 0x10000 && maxbytes >= 3)
	{
	...
	if (codepoint < 0x110000 && maxbytes >= 4)
	{
		...
		case 'u':
			if (srcend - src > 4)
			{
				...
				if (JSON_ReadHexNumber (src + 1, &codepoint))
				{
					...
					dst += JSON_WriteUTF8 (dst, 4, (unsigned int)codepoint);
					src += 4;
					continue;
				}
			}
