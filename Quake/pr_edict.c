...
const char *ED_ParseEdict (const char *data, edict_t *ent)
{
	...
	while (1)
	{
		...
		// HACK: allow truncation for long map-editor wad lists.
		// Some large modern maps exceed the token buffer here, but the field is only editor metadata.
...
static qboolean ED_IsSkillSelector (const edict_t *ent)
{
	...
	if (!strcmp (classname, "trigger_setskill") || !strcmp (classname, "target_setskill"))
		return true;
	if (!strcmp (classname, "info_command") && (int)ent->v.message != 0 &&
...
void ED_LoadFromFile (const char *data)
{
	...
	while (1)
	{
		...
		if (sv.mapchecks.active)
		{
			...
			else if (!strcmp (classname, "info_intermission"))
				sv.mapchecks.intermission++;
			else if (!strcmp (classname, "info_player_coop"))
				sv.mapchecks.coop_spawns++;
			else if (!strcmp (classname, "info_player_deathmatch"))
