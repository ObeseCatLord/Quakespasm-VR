...
int main(int argc, char *argv[]) {
  ...
  if (isDedicated) {
    ...
    while (1) {
      ...
      if ((!VID_HasMouseOrInputFocus() || cl.paused) && !vr_enabled.value &&
          cls.state != ca_connected) {
        SDL_Delay(16);
      }
      /* If we're minimised, sleep a bit more */
      if (VID_IsMinimized() && !vr_enabled.value && cls.state != ca_connected) {
        scr_skipupdate = 1;
        SDL_Delay(32);
      } else {
        scr_skipupdate = 0;
      ...
      if (time < sys_throttle.value && !cls.timedemo &&
          !vr_enabled.value && cls.state != ca_connected)
        SDL_Delay(1);
