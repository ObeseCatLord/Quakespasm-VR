...
static double filter_energy(int voice_kind, float hz, float obstruction, float radio_filter)
{
    sa_renderer_t *r = SA_Create(1, voice_kind);
    sa_settings_t settings = {.radio_gain = 1, .voice_distance = 768,
        .radio_filter = radio_filter, .occlusion = 1};
    sa_source_t src = {.active = 1, .generation = 1, .gain = 0.2f,
        .kind = voice_kind ? SA_VOICE : SA_POSITIONAL, .obstruction = obstruction};
    float pcm[SA_BLOCK], out[SA_BLOCK * 2];
    int16_t input[SA_BLOCK];
    sa_sample_t wave = {pcm, SA_BLOCK, 0};
    double sum = 0;
    int b, i;
    assert(r); SA_SetSettings(r, &settings); SA_SetListener(r, &listener);
    src.sample = &wave; SA_SetSource(r, 0, &src);
    for (b = 0; b < 160; ++b) {
        for (i = 0; i < SA_BLOCK; ++i) {
            pcm[i] = 0.4f * sinf(6.2831853f * hz * (b * SA_BLOCK + i) / SA_RATE);
            input[i] = (int16_t)(pcm[i] * 32767);
        }
        if (voice_kind) assert(SA_WriteVoice(r, 0, input, SA_BLOCK) == SA_BLOCK);
        SA_Render(r, out, SA_BLOCK);
        if (b > 80) sum += energy(out, SA_BLOCK, 0);
    }
...
int main(int argc, char **argv)
{
    sa_renderer_t *r;
    sa_stats_t stats;
    sa_settings_t settings = {.hrtf = 1, .radio_gain = 0.45f, .voice_distance = 768};
    int16_t voice[960];
    int i, j;
    double right, left;
    SDL_Thread *thread;
    ...
    /* Frequency-response checks measure the rendered paths, not coefficient formulas. */
    {
        double pass = filter_energy(1, 1000, 0, 1) / filter_energy(1, 1000, 0, 0);
        double low = filter_energy(1, 60, 0, 1) / filter_energy(1, 60, 0, 0);
        double high = filter_energy(1, 12000, 0, 1) / filter_energy(1, 12000, 0, 0);
        assert(pass > 0.3 && pass < 1.1); assert(low < 0.01); assert(high < 0.04);
        assert(fabs(filter_energy(1, 1000, 1, 1) / filter_energy(1, 1000, 0, 1) - 1) < 1e-5);
        low = filter_energy(0, 100, 1, 0) / filter_energy(0, 100, 0, 0);
        high = filter_energy(0, 8000, 1, 0) / filter_energy(0, 8000, 0, 0);
        assert(low > 0.05 && low < 0.2); assert(high < low * 0.02);
    }
