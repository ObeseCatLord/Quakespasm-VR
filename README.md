...
| `sv_replacement_packetdup` | `1` | Sends this many same-sequence duplicates of replacement-delta datagrams to remote clients by default. Duplicates are dropped as stale if both copies arrive, but they can hide a single downstream UDP loss before replacement-delta resend recovery has to catch up. |
| `sv_replacement_packetdup_all` | `1` | When `1`, duplicates every replacement packet, including continuation packets from large frames. Set to `0` to duplicate only the first state/ack packet and reduce downstream bandwidth. |
