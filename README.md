...
| `cl_move_redundancy` | `12` | Sends previous QSS-style `clc_move` records with each new move so upstream loss bursts do not lose input frames. Command duration is derived from the QSS-style server-time stamp instead of a separate msec byte. |
| `cl_move_packetdup` | `0` | Sends duplicate movement datagrams when manually enabled for lossy/low-rate links. It defaults off at the QSS-style 72 Hz send cadence because `cl_move_redundancy` already carries prior commands and duplicating every datagram can create upstream packet pressure. |
...
| `sv_replacement_packetdup` | `0` | Sends same-sequence duplicates of replacement-delta datagrams when manually enabled for lossy links. It defaults off to match QSS-M's lower-bandwidth replacement-delta behavior and avoid creating downstream packet pressure. |
| `sv_replacement_packetdup_all` | `0` | When `1`, duplicates every replacement packet, including continuation packets from large frames. Leave off unless deliberately testing duplicate downstream snapshots. |
