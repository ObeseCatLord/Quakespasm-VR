...
| `sv_pmove_legacy` | `0` | Applies server PMove to normal QuakeC mods that do not define `SV_RunClientCommand`, replacing legacy server-authoritative movement while preserving the mod's `PlayerPreThink`/`PlayerPostThink` hooks. Leave this off for QSS-M-style compatibility unless a mod is known to predict cleanly without `SV_RunClientCommand`. |
...
| `sv_moveack_independent` | `1` | Sends a tiny standalone movement-ack/prediction-state datagram before replacement-delta snapshots. This keeps PMove correction current when a large entity update is lost or delayed. |
