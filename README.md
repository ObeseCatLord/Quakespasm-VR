...
| `sv_replacement_packetdup` | `1` | Sends this many same-sequence duplicates of the first replacement-delta datagram to remote clients by default. The duplicate is dropped as stale if both copies arrive, but it can hide a single downstream UDP loss on the state/ack packet. |
| `sv_replacement_packetdup_all` | `0` | When `0`, only the first replacement-delta packet in an update is duplicated. This keeps loss protection on the state/ack packet while avoiding duplicate bandwidth for continuation/temp packets. Set to `1` to duplicate every replacement packet. |
